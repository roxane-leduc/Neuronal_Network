...
int Matrix::elementGrand(){
    ...
    for(int i=1;i<nbRows;i++){
        if(weight[i][0] < weight[res][0]) res = i;
    }
    return res;
}
    ...
    for(int i=0;i<nbRows;i++){
        weight[i] = new double[nbColumns];
            for(int j=0;j<nbColumns;j++){
                ...
                if(i==nbRowsTmp) weight[i][j] = 0;  
                // Sinon on prend les poids du tableau temporaire
                else weight[i][j] = tmp[i][j];  
        }
    }
    ...
    for(int i=0;i<nbRows;i++){
        weight[i] = new double[nbColumns];
            for(int j=0;j<nbColumns;j++){
                ...
                if(i==nbRowsTmp) weight[i][j] = Ligne.weight[0][j];  
                // Sinon on prend les poids du tableau temporaire
                else weight[i][j] = tmp[i][j];  
        }
    }
    ...
    for(int i=0;i<nbRows;i++){
        weight[i] = new double[nbColumns];
            for(int j=0;j<nbColumns;j++){
                ...
                if(j==nbColumnTmp) weight[i][j] = Colonne.weight[i][0];  
                // Sinon on prend les poids du tableau temporaire
                else weight[i][j] = tmp[i][j];  
        }
    }
